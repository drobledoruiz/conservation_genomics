...
table <- data.frame(n0=n0,n1=n1,n2=n2)
#-----------------------------------------------------
...
HW.chsqtest <- function(table,cc=cc){
  ...
  for(i in 1:nrow(table)){
      ...
      table[i,'En2'] <- chsq[3]
      table[i,'chsq']<- chsq[4]
      table[i,'p.value'] <- pchisq(chsq[4], 1, lower.tail=FALSE)
  }
  return(table)
}
#----------------------------------------------------
table.filter <- table[table$n1/(table$n0 + table$n1 + table$n2) >= 0.5,] 
